...
//	memcpy(m_Buffer, 0, m_Width*m_Height*sizeof(Pixel));
}
...
/*void Surface::CopyToAlphablend( Surface* a_Dst, int a_X, int a_Y, int alphaD) {
	...
	for ( int y = 0; y < srcheight; y++ ) {
		for(int x = 0; x < srcwidth; x++) {
			float a_alpha = ((src[x]&ALPHAMASK)>>24) / 255.f;
			a_alpha += float(alphaD)/255.f;
			if(a_alpha < 0.f) a_alpha = 0.f;
			if(a_alpha > 1.f) a_alpha = 1.f;
			dst[x] = int((1.f-a_alpha)*((dst[x]&REDMASK)>>16) + a_alpha*((src[x]&REDMASK)>>16))<<16 |\
					 int((1.f-a_alpha)*((dst[x]&GREENMASK)>>8) + a_alpha*((src[x]&GREENMASK)>>8))<<8 |\
					 int((1.f-a_alpha)*(dst[x]&BLUEMASK) + a_alpha*(src[x]&BLUEMASK));
		}
		dst += dstpitch;
		src += srcpitch;
	}
...
void Surface::CopyToAlphablend( Surface* a_Dst, int a_X, int a_Y, int alphaD) {
	...
	for ( int y = 0; y < srcheight; y++ ) {
		for(int x = 0; x < srcwidth; x++) {
			dst[x] = AlphaBlendFast(dst[x], src[x], alphaD);
		}
		dst += dstpitch;
		src += srcpitch;
	}
...
void Surface::CopyAreaTo( Surface* a_Dst, int a_X, int a_Y , int src_x, int src_y, int width, int height)
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = m_Buffer;
	if ((src) && (dst)) 
...
void Surface::CopyAreaToColorkey( Surface* a_Dst, int a_X, int a_Y , int src_x, int src_y, int width, int height)
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = m_Buffer;
	if ((src) && (dst)) 
