...
{  
	...
	if (MessageBox(NULL, "Run in fullscreen?", "PRType 1.0", MB_YESNO) == IDYES) {
		SDL_SetVideoMode( SCRWIDTH, SCRHEIGHT, 24, SDL_OPENGL | SDL_FULLSCREEN);
	} else {
		SDL_SetVideoMode( SCRWIDTH, SCRHEIGHT, 24, SDL_OPENGL);
	}
	...
	else SDL_WM_SetCaption( "LOADING...", NULL );
